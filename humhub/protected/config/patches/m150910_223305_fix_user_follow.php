...
class m150910_223305_fix_user_follow extends Migration
{
    ...
    public function up()
    {
        $activities = (new \yii\db\Query())->select("activity.*, content.user_id, user_follow.id as follow_id")->from('activity')
                ...
                ->leftJoin('user_follow', 'activity.object_model=user_follow.object_model AND activity.object_id=user_follow.object_id::varchar AND user_follow.user_id=content.user_id')
